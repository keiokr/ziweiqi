...
def is_valid_url(url):
    ...
    regex = re.compile(
        r'^(?:(?:http|https|socks|socks4|socks5|socks5h))://'  # proxy scheme
...
def normalize_proxy_url(proxy):
    """Normalize proxy input to a valid aiohttp proxy URL."""
    ...
    return proxy if is_valid_url(proxy) else None


def is_socks5_proxy_url(url):
    proxy = (url or "").strip().lower()
    return proxy.startswith("socks5://") or proxy.startswith("socks5h://")


def get_single_query_retry_times():
    return max(1, int(getattr(getattr(config, "captcha", object()), "retry_times", 60) or 1))


def get_batch_keyword_retry_limit():
    return max(1, int(getattr(getattr(config, "system", object()), "batch_keyword_retry_limit", 100) or 100))


def get_batch_retry_delay_seconds(retry_index):
    retry_limit = get_batch_keyword_retry_limit()
    current_index = max(1, min(int(retry_index or 1), retry_limit))
    if retry_limit == 1:
        return 10.0

    progress = (current_index - 1) / (retry_limit - 1)
    return round(10 + (60 - 10) * progress, 2)


def get_batch_query_limit_config():
    system_config = getattr(config, "system", object())
    limit_config = getattr(system_config, "batch_query_limit", None) or object()
    return {
        "enable": bool(getattr(limit_config, "enable", True)),
        "request_interval_ms": max(0, int(getattr(limit_config, "request_interval_ms", 6000) or 0)),
        "max_per_minute": max(0, int(getattr(limit_config, "max_per_minute", 10) or 0)),
        "window_seconds": max(1, int(getattr(limit_config, "window_seconds", 60) or 60)),
    }


def get_effective_batch_step_delay_seconds():
    """Compute the effective delay between batch items from both interval and per-minute cap."""
    cfg = get_batch_query_limit_config()
    if not cfg["enable"]:
        return 0

    delay_seconds = cfg["request_interval_ms"] / 1000
    if cfg["max_per_minute"] > 0:
        delay_seconds = max(delay_seconds, cfg["window_seconds"] / cfg["max_per_minute"])
