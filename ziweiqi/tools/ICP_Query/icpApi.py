...
def get_browser_access_host():
    """返回适合浏览器直接访问的地址。"""
    host = getattr(getattr(config, "system", object()), "host", "127.0.0.1")
    if host in ("0.0.0.0", "::", "", None):
        return "127.0.0.1"
    return host


def get_browser_access_url():
    return f"http://{get_browser_access_host()}:{config.system.port}"


async def open_browser_on_startup(app):
    """服务启动后自动打开默认浏览器。"""
    if not config.system.web_ui:
        return

    async def delayed_open():
        await asyncio.sleep(1)
        url = get_browser_access_url()
        try:
            webbrowser.open(url, new=2)
            logger.info(f"已打开默认浏览器: {url}")
        except Exception as e:
            logger.warning(f"打开默认浏览器失败: {e}")
...
def create_app():
    ...
    elif not getattr(config.proxy.tunnel, "enable", False):
    ...
    if config.system.web_ui:
        app.on_startup.append(open_browser_on_startup)
