...
5.  脚本说明
     ...
     step16_merge_filter_ip.py  合并并过滤 IP 结果<过滤cdn ip等等>。
     step17_run_masscan.py  运行 masscan 扫描端口<进行预端口20个判断开放10个以上的>。
     step18_filter_masscan_ip.py 过滤 masscan 扫描得到的泛端口开放IP。
     step19_merge_ports.py  合并各类端口结果。
     step20_run_fscan_port_fofa.py   用 FOFA 相关端口集跑 fscan。
     step20_run_fscan_port_4000.py 用 4000 端口范围跑 fscan。
     step20_run_fscan_port_all.py  用全端口范围跑 fscan。
...
class ZiWeiQiGui(tk.Tk):
    def __init__(self):
        ...
        self.idle_timeout_var = tk.StringVar(value="3000")
    ...
    def _build_ui(self):
        ...
        for index in range(10):
            controls.columnconfigure(index, weight=1 if index in (1, 3, 5, 7, 9) else 0)

        ttk.Label(controls, text="子域名字典").grid(row=0, column=0, sticky="w")
        ...
        self.mode_box.grid(row=0, column=1, sticky="ew", padx=(6, 12))

        ttk.Label(controls, text="控股（默认51）").grid(row=0, column=2, sticky="w")
        ttk.Entry(controls, textvariable=self.invest_var, width=8).grid(row=0, column=3, sticky="ew", padx=(6, 12))
        ttk.Label(controls, text="1子公司/2孙公司").grid(row=0, column=4, sticky="w")
        ttk.Entry(controls, textvariable=self.deep_var, width=8).grid(row=0, column=5, sticky="ew", padx=(6, 12))
        ttk.Label(controls, text="超时").grid(row=0, column=6, sticky="w")
        ttk.Entry(controls, textvariable=self.delay_var, width=8).grid(row=0, column=7, sticky="ew", padx=(6, 12))
        ttk.Label(controls, text="空闲超时").grid(row=0, column=8, sticky="w")
        ttk.Entry(controls, textvariable=self.idle_timeout_var, width=8).grid(row=0, column=9, sticky="ew", padx=(6, 0))
    ...
    def _build_job_snapshot(self, include_enscan: bool, include_pipeline: bool):
        ...
        idle_timeout = self._validate_number(self.idle_timeout_var.get(), "空闲超时", allow_zero=True)
        steps = []
        ...
        if include_pipeline:
            script = PIPELINES.get(self.mode_var.get().strip())
            if script is None:
            ...
            steps.append({
                "name": f"资产测绘模式 {self.mode_var.get().strip()}",
    ...
    def _worker_wrapper(self, title: str, snapshot):
        try:
            for step in snapshot["steps"]:
                self._run_process(step["name"], step["command"], Path(step["cwd"]), snapshot["idle_timeout"])
    ...
    def _run_process(self, step_name: str, command, cwd: Path, idle_timeout: int):
        creationflags = getattr(subprocess, "CREATE_NO_WINDOW", 0)
        ...
        while True:
            ...
            if now - last_heartbeat_at >= HEARTBEAT_SECONDS:
                ...
                self.log_queue.put(("status", f"正在执行 {step_name} | 已运行 {elapsed}s | 空闲 {idle_for}s"))
                last_heartbeat_at = now

            if idle_timeout > 0 and now - last_output_at >= idle_timeout:
                self._terminate_process_tree(process)
                raise RuntimeError(f"{step_name} 连续 {idle_timeout} 秒没有输出，已被终止。")
