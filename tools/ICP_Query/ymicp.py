...
from utils import is_socks5_proxy_url
...
class beian:
    def __init__(self):
        ...
        self.request_interval_ms = max(0, int(getattr(getattr(config, 'system', object()), 'request_interval_ms', 1200) or 0))
        self.retry_backoff_ms = max(0, int(getattr(getattr(config, 'system', object()), 'retry_backoff_ms', 1500) or 0))
    ...
    async def _throttle_request(self, label=""):
        """统一平滑外部请求，避免短时间内连续打满目标站点。"""
        interval = self.request_interval_ms / 1000
        if interval <= 0:
            return

        if self._request_gate_lock is None:
    ...
    async def sleep_retry_backoff(self, retry_count=1):
        backoff = self.retry_backoff_ms / 1000
        if backoff <= 0:
            return
        await asyncio.sleep(backoff * max(1, min(retry_count, 3)))
    ...
    async def get_session(self, proxy=""):
        ...
        # 为每个session创建独立的连接器
        connector = ProxyConnector.from_url(proxy, ssl=False) if proxy and is_socks5_proxy_url(proxy) else await self._get_connector(local_ipv6)
    ...
    def _proxy_request_kwargs(self, proxy=""):
        if proxy and is_socks5_proxy_url(proxy):
            return {}
    ...
    async def get_token(self, proxy=""):
        ...
        try:
            async with self.get_session(proxy) as session:
                ...
                await self._throttle_request("auth_token")
                async with session.post(self.url, data=auth_data, headers=base_header, **self._proxy_request_kwargs(proxy)) as req:
    ...
    async def get_cookie(self, proxy=""):
        async with self.get_session(proxy) as session:
            await self._throttle_request("home_cookie")
            async with session.get(self.home, headers=self.cookie_headers, **self._proxy_request_kwargs(proxy)) as req:
                res = await req.text()
    ...
    async def check_img(self, proxy=""):
        ...
        try:
            ...
            try:
                async with self.get_session(proxy) as session:
                    await self._throttle_request("captcha_image")
                    async with session.post(self.getCheckImage, data=data, headers=base_header, **self._proxy_request_kwargs(proxy)) as req:
                        res = await req.json()
            except Exception as e:
            ...
            async with self.get_session(proxy) as session:
                await self._throttle_request("captcha_verify")
                async with session.post(self.checkImage, data=check_data, headers=base_header, **self._proxy_request_kwargs(proxy)) as req:
                    res = await req.text()
    ...
    async def getbeian(self, name, sp, pageNum, pageSize, proxy=""):
        ...
        if getattr(getattr(config, 'captcha', object()), 'enable', False):
            ...
            async with self.get_session(proxy) as session:
                await self._throttle_request("beian_query")
    ...
    async def getblackbeian(self, name, sp, proxy=""):
        ...
        if getattr(getattr(config, 'captcha', object()), 'enable', False):
            ...
            async with self.get_session(proxy) as session:
                ...
                if hasattr(session, '_connector') and hasattr(session._connector, '_local_addr'):
                    current_ip = session._connector._local_addr[0] if session._connector._local_addr else None
                await self._throttle_request("black_query")
                async with session.post((self.blackqueryByCondition if sp == 0 else self.blackappAndMiniByCondition),
                                         data=ujson.dumps(info, ensure_ascii=False),
                                         headers=base_header, **self._proxy_request_kwargs(proxy)) as req:
                    res = await req.text()
