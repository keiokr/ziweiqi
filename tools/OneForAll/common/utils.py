...
def save_to_file(path, data):
    ...
    """
    try:
        with open(path, 'w', errors='replace', newline='') as file:
            file.write(data)
            return True
    except TypeError:
...
def call_massdns(massdns_path, dict_path, ns_path, output_path, log_path,
    ...
    if quiet_mode:
        cmd.append('--quiet')
    cmd.extend([
        '--status-format', str(status_format),
        '--processes', str(process_num),
        '--socket-count', str(socket_num),
        '--hashmap-size', str(concurrent_num),
        '--resolvers', str(ns_path),
        '--resolve-count', str(resolve_num),
        '--type', str(query_type),
        '--flush',
        '--output', 'J',
    ...
    logger.log('DEBUG', f'Run command {subprocess.list2cmdline(cmd)}')
