...
# 兼容旧版 exrex 在 Python 3.11+ 中执行 `from re import sre_parse, U` 的导入方式。
# Python 3.9 中 re.sre_parse 仍可用；Python 3.11+ 移除了这个 re 模块属性，
# 但独立的 sre_parse 模块仍存在，因此在导入 exrex 前补回该属性。
