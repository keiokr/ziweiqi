...
def parse_targets(text: str) -> list[InputTarget]:
    ...
    if not candidates:
        raise ValueError("请至少输入一个根域名或 IP。")
...
class DomainIpFilterGui:
    def __init__(self, root: Tk) -> None:
        self.root = root
        self.root.title("根域名 / IP URL 保留工具")
        ...
        self.url_file_var = StringVar(value="url.txt")
    ...
    def _build_ui(self) -> None:
        ...
        title = ttk.Label(main, text="result.txt / url.txt 保留工具", font=("Microsoft YaHei UI", 18, "bold"))
        title.pack(anchor="w")

        desc = ttk.Label(
            main,
            text=(
                "输入区只支持根域名或 IPv4，一行一个，也可以导入文本文件。"
                "程序会从 result.txt 中命中目标，再从 url.txt 中按当前模式直接生成一个结果文件。"
            ),
            wraplength=1180,
            justify="left",
        )
